...
- HomeHarvest is intentionally left unpinned (`homeharvest>=0.4.0`) so each
  run picks up the latest release. When Realtor.com changes its API or
  tightens blocking, the fix arrives as a HomeHarvest update — pinning would
  mean staying broken until someone bumped the version by hand.
